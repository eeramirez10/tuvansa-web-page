...
import { filters, productos } from './productosItems';
...
export const Productos2 = () => {
    ...
    const handleFilterProducts = (item) => {
        ...
        const filterproductos = productos.filter(productos => productos.filter !== item.filter);


        filterproductos.forEach(project => {
    ...
    return (
        <div className="section-full p-t80 p-b50 mobile-page-padding">
            <div className="container">
                ...
                <div  >
                    {
                        productos.map((item, index) => <Producto key={index} item={item} />)
                    }
                </div>
            </div>
        </div>
